...
| -------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| Zero-duration weapons can set active flags without natural `BT_WPN_OFF`.                                                         | Model one-shot effects explicitly in Rust, but add legacy behavior notes and tests for visible outcomes.             |
...
| `The Blind Cleric` description says it bombs a region, but code removes about half of all removable blocks.                      | Treat implementation as authoritative for compatibility tests; consider text update later.                           |
| `Twilight` and `Gimp` affect existing blocks only and have no undo path.                                                         | Preserve as one-shot board mutations.                                                                                |
...
| Piece-probability weapons mutate probabilities directly rather than recomputing from the active weapon set.                      | Preserve visible legacy stacking/restoration behavior until compatibility tests make a safer normalization possible. |
...
| Funds/scoring/lines      | `usr/src/game/BTScore.*`, `usr/src/game/BTScoreManager.*`, `usr/src/game/BTLine.H`, `usr/src/db/BTGameStats.*`                                                                         | Local/opponent score snapshots, funds, line counts, bazaar trigger, game stats.                                  | `battletris-core::score`, `battletris-db::stats`.                                           |
...
| Remote weapons are queued, then flushed after current piece placement/scoring/recon and before next-piece creation. | `usr/src/game/BTCommManager.C:419-424`, `usr/src/game/BTCommManager.C:573-589`, `usr/src/game/BTGame.C:776-803` | Preserve timing in deterministic core.                       |
